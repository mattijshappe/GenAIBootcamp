...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "mattijsHappe_38Srv.ProductFAQ")
...
try {
    ...
    productFAQ = await SELECT.one('MattijsHappe_38.ProductFAQ').where({ ID: productFAQID }).forUpdate();
    if (!productFAQ) {
...
try {
    ...
    await UPDATE('MattijsHappe_38.ProductFAQ').set({ embedding: embedding }).where({ ID });
